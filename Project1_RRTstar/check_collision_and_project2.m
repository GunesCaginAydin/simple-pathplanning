function [new_config, col_check] = check_collision_and_project2(parent_collision ,parent, config, config_space)
    ...
    while true
        ...
        if config_space(config(1),config(2)) == 1 % initial check for random point on obstacles
            ...
            elseif parent_collision == 0
                ...
                rr = rl:rh; cc = cl:ch;
                %collisions = cell(3,3); % collision collection
                for pr = 1:length(rr)
                    for pc = 1:length(cc)
                        if any(configc_run == cc(pr),'all') && any(configr_run == rr(pc),'all') && config_space(rr(pr),cc(pc)) == 1
                            %collisions{pr,pc} = [rr(pr),cc(pc)];
                            new_config = [rr(pr),cc(pc)];
                            col_check = 1;
                            %%%%%%%%%%% CONSIDER REDUCTION WEIGHT AGAIN
                            return
                        end
                    end
                end
            elseif parent_collision == 1
                %config = randi(rs,cs);
                %continue
                new_config = NaN;
                col_check = NaN;
            ...
            %return
                ...
                % [ri,ci] = find(config_space(configxy(:,1),configxy(:,2)) == 0);
                % config_dub = configxy(ind - 1);
                % new_config = randi(config_dub);

                                %Idx = rangesearch(X,Y,1.4,'Distance','seuclidean','Scale',iqr(X));
                %config_dub = closest_ind_to_rici;