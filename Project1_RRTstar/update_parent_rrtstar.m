function [parent_update, parent_change, cost_min, dist_min, min_par_indx, col, parent_idx_no_col, parent_list_no_col] = update_parent_rrtstar(tev, neigh_parent_list, neigh_parent_idx,fconfig_spc) % possibility to write a collision check function
    parent_change = 0;
    col = 0;
    tree_len = length(tev);
    ...
    if isempty(parent_list_no_col)
        col = 1; parent_update = 0; cost_min = 0; min_par_indx = 0; parent_idx_no_col = 0; dist_min = 0;
        return
    end
    ...
    dist_min = cost_add_no_col(min_par_indx);
